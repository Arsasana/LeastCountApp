...
//write a fun for updating winning score
	$scope.newScore;
	$scope.errGameScr="";
	
	
	$scope.updateGameScore = function(){
		console.log("In update fun ");
		var newScr = $scope.newScore;
		var maxScore = $scope.totalScores[0];
		console.log("max scr : ",maxScore," new Scr",newScr);
		...
		if(newScr > maxScore)
			{
			playerNameService.addGameScore(newScr);
			$scope.gameScore = playerNameService.getGameScore();
			console.log("Updated Game Score :",$scope.gameScore);
			}
		else{
			//not allow
			$scope.errGameScr = "enter the score more than the highest total of a player";
	...
	$scope.addNewPlayer = function(){
		...
		$scope.playerNames.push($scope.newPlayer);
		console.log("######",$scope.playerNames);
		var j =1;
		if($scope.inputData.length === 0 ){
			console.log("in add payer IFIF",JSON.stringify($scope.inputData));
			$scope.rowArr = chunk($scope.inputData,$scope.playerNames.length);
			$scope.colArr.push($scope.playerNames.length-1);
		}else{
			for(var i = 0 ; i < ($scope.inputData.length+1) ; i++){
				if(i === ($scope.playerNames.length * j)-1){
					temp.push("NA");
				//	temp[$scope.playerNames.length].push($scope.inputData[$scope.playerNames.length]);
					
					temp.push($scope.inputData[i]);
					j++
				}else{
					if(i===($scope.inputData.length))
						temp.push("NA");
					else
					temp.push($scope.inputData[i]);
				}
			...
			var newPlayerScore = [];
			$scope.playerSet.push({Name : $scope.newPlayer,Scores:[]});
			for(var i = 0; i < j;i++){
				newPlayerScore.push("NA");
				$scope.playerSet[$scope.playerNames.length-1].Scores.push("NA");
			}
			
				
			console.log("new Player Json playerSet",JSON.stringify($scope.playerSet));
			console.log("in add payer",JSON.stringify(temp));
			//$scope.inputData = "";
			$scope.inputData = temp;
			console.log("inputData",JSON.stringify($scope.inputData));
			$scope.rowArr = chunk($scope.inputData,$scope.playerNames.length);
			$scope.colArr.push($scope.playerNames.length-1);
		}
		};
	...
	$scope.add = function(){
		...
		console.log("Pravveeen ",JSON.stringify($scope.inputData));
		$scope.rowArr = chunk($scope.inputData,$scope.playerNames.length);
